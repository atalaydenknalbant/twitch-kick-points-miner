...
func TestStreakPriorityLimit(t *testing.T) {
	if streakPriorityMinutesBase != 15 || streakPriorityMinutesExtended != 15 {
		t.Fatalf("streak priority windows got %.0f and %.0f, want 15 and 15", streakPriorityMinutesBase, streakPriorityMinutesExtended)
	}
	m := &Miner{}
	now := time.Now()
...
func TestPickStreamersToWatchIncludesNewlyOnlineChannel(t *testing.T) {
	streamer := &entities.Streamer{
		Username: "fresh",
		IsOnline: true,
		OnlineAt: time.Now(),
	}
	m := &Miner{watchPriorities: defaultWatchPriorities()}

	got := m.pickStreamersToWatch([]*entities.Streamer{streamer})
	if len(got) != 1 || got[0] != streamer {
		t.Fatalf("newly online channel should be watchable immediately")
	}
}
